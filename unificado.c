...
/* FUNÇÃO MODIFICADA: Agora aceita coordenadas como "1A", "2B", etc. */
int get_human_move(char board[BOARD_ROWS][BOARD_COLS], char current_piece) {
    char input[10];
    int col_index = -1;
    int valid_input = 0;
    ...
    while (!valid_input) {
        printf("Jogador %c, escolha sua jogada (ex: 1A, 2B, 3C, etc): ", current_piece);
        
        if (scanf("%9s", input) != 1) {
            printf("Entrada inválida. Por favor, digite uma coordenada como '1A'.\n");
            while (getchar() != '\n');
            continue;
        }

        /* Processa a entrada: pode ser apenas letra (A-G) ou coordenada (1A, 2B, etc) */
        if (strlen(input) == 1) {
            /* Entrada antiga: apenas letra */
            col_index = char_to_col_index(input[0]);
        } else {
            /* Nova entrada: coordenada como "1A" - usa a letra */
            col_index = char_to_col_index(input[1]);
        }

        if (!is_valid_col_index(col_index)) {
            printf("Coordenada inválida! Use letras de A a %c.\n", 'A' + BOARD_COLS - 1);
            continue;
        }
...
/* FUNÇÃO MODIFICADA: Também aceita coordenadas completas */
int ui_get_human_move(const char board[BOARD_ROWS][BOARD_COLS], char current_piece) {
    char input[10];
    int column_index;
    for (;;) {
        printf("Jogador %c, escolha sua jogada (ex: 1A, 2B ou apenas A, B, C...): ",
               current_piece);
        
        if (scanf("%9s", input) != 1) {
            printf("Entrada inválida. Por favor, digite uma coordenada.\n");
            clear_input();
            continue;
        }
        clear_input();

        /* Processa tanto "A" quanto "1A" */
        if (strlen(input) == 1) {
            column_index = letter_to_column_index(input[0]);
        } else {
            column_index = letter_to_column_index(input[1]); /* Pega a letra de "1A" */
        }

        if (column_index < 0 || column_index >= BOARD_COLS) {
